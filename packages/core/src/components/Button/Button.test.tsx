...
test("applies the primary variant class by default", async () => {
  ...
  await expect.element(button).toHaveClass("poiui-button");
  await expect.element(button).toHaveClass("poiui-button--primary");
});
...
test("applies the secondary variant class when specified", async () => {
  ...
  await expect.element(button).toHaveClass("poiui-button--secondary");
});
