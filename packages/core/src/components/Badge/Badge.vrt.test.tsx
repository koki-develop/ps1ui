...
const THEMED_STATIC_CASES = THEMES.flatMap((theme) => STATIC_CASES.map((c) => ({ theme, ...c })));

describe("Badge VRT", () => {
  // Static badges — the (variant × color) matrix at rest. This is the
  // full colour surface; any token / mix shift shows here first.
  test.for(THEMED_STATIC_CASES)(
    "theme=$theme / variant=$variant / color=$color / default",
    async ({ theme, variant, color }) => {
      ...
      await expect
        ...
        .toMatchScreenshot(`${theme}-${variant}-${color}`);
    },
  );
  ...
  test.for(THEMES.map((theme) => ({ theme })))(
    "theme=$theme / matrix side-by-side",
    async ({ theme }) => {
      const screen = await render(
        <VrtFrame theme={theme}>
      ...
      await expect.element(screen.getByTestId("vrt-frame")).toMatchScreenshot(`${theme}-matrix`);
    },
  );
  ...
  const INTERACTIVE_CASES = THEMES.flatMap((theme) =>
    VARIANTS.flatMap((variant) => INTERACTIVE_STATES.map((state) => ({ theme, variant, state }))),
  );
  test.for(INTERACTIVE_CASES)(
    "theme=$theme / as=button / variant=$variant / state=$state",
    async ({ theme, variant, state }, ctx) => {
      ...
      await withPseudoStateFor('[data-testid="vrt-target"]', state, PSEUDO_STATES, async () => {
        await expect
          .element(screen.getByTestId("vrt-frame"))
          .toMatchScreenshot(`${theme}-interactive-${variant}-${state}`);
      });
    },
  );
  ...
  test.for(THEMES.flatMap((theme) => VARIANTS.map((variant) => ({ theme, variant }))))(
    ...
    async ({ theme, variant }) => {
      ...
      await expect
        .element(screen.getByTestId("vrt-frame"))
        .toMatchScreenshot(`${theme}-as-a-${variant}`);
    },
  );
  ...
  test.for(THEMES.map((theme) => ({ theme })))(
    "theme=$theme / leading slot aligns inline with the label",
    async ({ theme }) => {
      const screen = await render(
        <VrtFrame theme={theme}>
          <Badge
      ...
      await expect.element(screen.getByTestId("vrt-frame")).toMatchScreenshot(`${theme}-leading`);
    },
  );
  ...
  test.for(THEMES.flatMap((theme) => SIZES.map((size) => ({ theme, size }))))(
    "theme=$theme / size=$size / default state",
    async ({ theme, size }) => {
      const screen = await render(
        <VrtFrame theme={theme}>
      ...
      await expect
        .element(screen.getByTestId("vrt-frame"))
        .toMatchScreenshot(`${theme}-size-${size}`);
    },
  );
  ...
  test.for(THEMES.map((theme) => ({ theme })))(
    "theme=$theme / leading slot spacing scales with size",
    async ({ theme }) => {
      const screen = await render(
        <VrtFrame theme={theme}>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
      ...
      await expect
        .element(screen.getByTestId("vrt-frame"))
        .toMatchScreenshot(`${theme}-sizes-leading-row`);
    },
  );
});
