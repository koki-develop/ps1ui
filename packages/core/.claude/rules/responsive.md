---
paths:
  - "**/src/components/{Anchor,Container,Grid,GridItem,Stack,Heading,Text,PS1Root}/**"
...
`Responsive<T>` props resolve via `resolveResponsive(value, "--_<comp>-<axis>", transform)` (`src/utils/responsive.ts`) into inline `--_*` custom properties; component CSS cascades them through `@container` breakpoint blocks (sm 40rem / md 48rem / lg 64rem / xl 80rem). Offered on Container, Grid, GridItem, Stack, Heading, Text, Anchor. Read `responsive.ts` and an existing component's CSS for the mechanism; Heading's `withResponsiveBase` handles prop-dependent base defaults.
