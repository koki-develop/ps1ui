...
- `pnpm check:responsive-property-coverage` — enumerates every `resolveResponsive(_, "PREFIX", …)` literal-arg call across `src/components/**/*.tsx` and STRUCTURALLY enforces that the sibling `.css` registers all 5 `@property PREFIX-<bp>` blocks with `inherits: false`; also flags orphan `@property --_*` blocks with no matching resolveResponsive call. See "Non-inheriting input variables (load-bearing)" under "Responsive props" for the invariant this protects.
...
- **`does not inherit outer's per-breakpoint input vars`** — a `test.for(AXES × BREAKPOINTS_NON_BASE)` matrix with the axes table declared as `Record<AxisUnion, LeakCase>` via `as const satisfies` so a new axis added to the component's TSX (that grows the axis union) fails compilation until the table gains an entry. Each row: outer sets `{ base: <safe>, [bp]: <distinct> }`; inner sets ONLY a scalar `base` (or omits the prop). Rendered inside a `<div style={{ containerType: "inline-size", width }}>` wrapper where `width` is 700 / 900 / 1200 / 1400 per sm/md/lg/xl (so the outer's OWN @container query fires at each target breakpoint and outer's inline-size exceeds the threshold). Assert inner's computed style reflects its OWN base, NOT the distinct outer `[bp]` value. Together with the `check:responsive-property-coverage` structural gate, this catches both a forgotten `@property` block AND a wrong `inherits:` value at authoring time.
